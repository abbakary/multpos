...
(function ($) {
  ...
  function collectOrderIdsFromPage(){var e=new Set;document.querySelectorAll('a[href^="/orders/"]').forEach(function(t){var r=t.getAttribute("href").match(/\/orders\/(\d+)\//);r&&e.add(r[1])}),document.querySelectorAll("[data-order-id]").forEach(function(t){e.add(String(t.getAttribute("data-order-id")))});return Array.from(e)}function refreshOrderStatuses(){var e=collectOrderIdsFromPage();e.length&&fetch("/api/orders/statuses/?ids="+e.join(",")).then(function(e){return e.json()}).then(function(e){if(e&&e.success){var t=e.orders||{};Object.keys(t).forEach(function(e){var r=t[e];document.querySelectorAll('[data-order-id="'+e+'"], a[href="/orders/'+e+'/"], tr[data-order-id="'+e+'"]').forEach(function(e){var t=e.querySelector(".badge-status, .status-badge, .badge");if(t){t.textContent=r.status_display,t.className=(t.className||"").replace(/\b(bg-soft-[^\s]+|text-[^\s]+|badge-status-[^\s]+)\b/g,"").trim();var a=(r.status||"").replace("_","-");"in-progress"===a?t.classList.add("bg-soft-warning","text-warning"):"overdue"===a?t.classList.add("bg-soft-danger","text-danger"):"completed"===a?t.classList.add("bg-soft-success","text-success"):"cancelled"===a&&t.classList.add("bg-soft-secondary","text-secondary")}})})}}).catch(function(){})}function normalizeStatusSelects(){document.querySelectorAll('select option[value="created"]').forEach(function(e){e.textContent="Start"})}function enhanceOrderCompletionForm(){var e=document.querySelector('form[action*="/complete/"]');if(e){var t=e.querySelector('input[type="password"]');function r(){var t=e.querySelector('input[type="file"][name="signature_file"]'),r=e.querySelector('input[type="file"][name="completion_attachment"]');t&&t.files&&t.files.length||r&&r.files&&r.files.length&&(e.requestSubmit?e.requestSubmit():e.submit())}t&&(t.closest(".form-group")?t.closest(".form-group").style.display="none":t.style.display="none"),e.querySelectorAll('input[type="file"]').forEach(function(e){e.addEventListener("change",r)})}}function reorganizeSalesServiceForm(){var e=document.querySelector('input[name="intent"]:checked')||document.getElementById("registrationIntent"),t=e?(e.value||e.textContent||"").toLowerCase():"",r=document.getElementById("form-container")||document;if(r){var n="sales"===t||"purchase"===t||"sale"===t,o="service"===t,i="consultation"===t||"inquiry"===t||"inquiries"===t,l=["id_item_name","id_brand","id_quantity","id_tire_type","id_condition","id_description"],c=["id_inquiry_type","id_questions","id_contact_preference","id_follow_up_date"];if(i)return l.forEach(function(e){var t=r.querySelector("#"+e);if(t){var n=t.closest(".mb-3, .form-group, .col");n&&(n.style.display="none")}}),void c.forEach(function(e){var t=r.querySelector("#"+e);if(t){var n=t.closest(".mb-3, .form-group, .col");n&&(n.style.display="")}});if(n||o){var s=["id_item_name","id_brand","id_quantity","id_tire_type","id_condition","id_description"],d=r.querySelector("form")||r,a=r.querySelector("#step-3, #items-section, form .row, form .card-body")||d;if(a){var u=document.createElement("div");u.id="organized-sales-service",u.className="row g-3",s.forEach(function(e){var t=r.querySelector("#"+e);if(t){var n=t.closest(".mb-3, .form-group, .col")||t;u.appendChild(n),n.style.display=""}});var f=r.querySelector("#tire-addons, .tire-addons");f||(f=document.createElement("div"),f.className="col-12",f.innerHTML='<div class="card"><div class="card-header"><h6>Tire Service Add-ons</h6></div><div class="card-body d-flex flex-wrap gap-3"><label class="form-check-label"><input type="checkbox" class="form-check-input" name="addons_install"/> Install Tires</label><label class="form-check-label"><input type="checkbox" class="form-check-input" name="addons_balance"/> Wheel Balancing</label><label class="form-check-label"><input type="checkbox" class="form-check-input" name="addons_align"/> Wheel Alignment</label><label class="form-check-label"><input type="checkbox" class="form-check-input" name="addons_dispose"/> Old Tire Disposal</label></div></div>'),u.appendChild(f),document.getElementById("organized-sales-service")||a.prepend(u)}}}}
