...
def api_customer_groups_data_fixed(request):
    ...
    for customer_type, display_name in customer_types.items():
        # Base customer queryset for this type
        customers_qs = Customer.objects.filter(customer_type=customer_type)
        
        # Apply activity filter - database compatible
        if activity_filter == 'active':
            if is_mysql():
                ...
                customers_qs = customers_qs.filter(
                    orders__created_at__date__gte=start_datetime
                ).distinct()
        ...
        # Count orders for this customer type within period - database compatible
        if is_mysql():
        ...
        # Calculate revenue (sum of total_spent for customers with orders in period) - MySQL compatible
        if activity_filter == 'active' or activity_filter == 'all':
            revenue = customers_qs.filter(
                orders__created_at__gte=start_datetime,
                orders__created_at__lte=end_datetime
            ).distinct().aggregate(
